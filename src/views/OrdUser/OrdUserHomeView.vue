  ...
  <div class="commodity">
    <el-row>
      <el-col
        ...
        :key="index"
      >
        <el-card shadow="hover" class="animated-card" @click="navigateToCommodityDetail(item.id)">
  ...
  methods: {
    async fetchRecommendData() {
      await axios
        ...
        .then((response) => {
          ...
          this.userBasedArray = response.data
    ...
    async fetchHotRecommendData() {
      await axios.get('/api/recommend/valueBased').then((response) => {
        console.log(response.data)
        this.hotRecommendData = response.data
        this.showCommodityArray = this.hotRecommendData
      })
    },
    navigateToCommodityDetail(commodityId) {
      localStorage.setItem('showCommodityId', commodityId)
      this.$router.push('/home/orduser/commodityDetail')
    },
